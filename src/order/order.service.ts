...
export class OrderService {
    ...
    async newOrder(orderDto:NewOrderDto,user:jwtPayloadType){
        try {
            ...
            const orderExists = await this.orderRepository.findOne({ where: { clientToken } });
            if (orderExists) {
                return orderExists;
            }
            const newOrder = this.orderRepository.create({ items, clientToken, user: { id: user.sub } });
        ...
        try {
           ...
           const order = await this.orderRepository.findOne({ where: { id: orderId } });
            ...
            if(error.name ==='OptimisticLockVersionMismatchError' ){
                throw new ConflictException('The order was updated by another process. Please try again.');
            }
            throw new HttpException(error.message, error.status || 500);
        }
    }
